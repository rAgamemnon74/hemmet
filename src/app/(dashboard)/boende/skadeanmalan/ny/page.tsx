...
export default function NewDamageReportPage() {
  ...
  const createReport = trpc.damageReport.create.useMutation({
    onSuccess: async (report) => {
      if (attachments.length > 0) {
        await submitAttachments("DamageReport", report.id, attachments);
      }
      router.push(`/boende/skadeanmalan/${report.id}`);
    },
  });
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-6">
        <Link href="/boende/skadeanmalan"
          className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
          <ArrowLeft className="h-4 w-4" /> Tillbaka till felanmälningar
        </Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-lg border border-gray-200 bg-white p-6">
        {createReport.error && (
          <div className="rounded-md bg-red-50 p-3 text-sm text-red-700">{createReport.error.message}</div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">Allvarlighetsgrad</label>
        ...
        {/* Bilagor */}
        <AttachmentInput attachments={attachments} onChange={setAttachments} label="Bilagor (foton, underlag)" />
